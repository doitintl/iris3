...
class GCEIntegTest(BaseIntegTest):
    def __init__(self):
        super().__init__()
        resources_per_type = 2
        self.__sfxs = ["number" + str(i) for i in range(resources_per_type)]
    ...
    def _resource_creation_commands(
        ...
        return [
            [
                ...
                f"gcloud compute snapshots create snapshot{run_id}{sfx} --source-disk disk{run_id}{sfx} --source-disk-zone {gce_zone} --storage-location {region} --project {test_project}",
            ]
            for sfx in self.__sfxs
        ] + [
