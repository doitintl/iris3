...
class IntegTest:
    def __init__(self):
        ...
        self.__global_exit_code = 0

    def __set_failing(self):
        self.__global_exit_code = 1
    ...
    @classmethod
    @log_time
    def _describe_resources(cls, test_project, run_id, gce_zone) -> Union[List, bool]:
    ...
    def deploy_test_uninstall(self):
        ...
        (
            ...
            run_id,
            gce_zone,
            repeat_a_few_resource_types,
        ) = self.__configuration()
        self.__deploy(deployment_project)
        if not repeat_a_few_resource_types:
    ...
    def __create_check_and_delete_resources(
        ...
        try:
            succeed = self.__create_and_describe_resources(test_project, run_id, gce_zone)
            if not succeed:
                self.__set_failing()
        except:
            logging.exception("")

        self.clean_resources(deployment_project, test_project, run_id, gce_zone)
    ...
    def __configuration(cls):
        ...
        if len(sys.argv) > 4:
            repeat_a_few_resource_types = bool(sys.argv[4])
        else:
            repeat_a_few_resource_types = False
        ...
        cls.check_projects_exist(deployment_project, test_project)
        _ = run_command(f"gcloud config set project {test_project}")
        cls.remove_temporary_test_config_file()
        atexit.register(cls.remove_temporary_test_config_file)
        cls.fill_in_config_template(
    ...
    def __create_and_describe_resources(self, test_project, run_id, gce_zone) -> bool:
        self.create_resources(test_project, run_id, gce_zone)
        ...
        try:
            while time.time() - start < 120:  # Go at most 2 minutes
                ...
                labels_not_found: Union[ List,bool] = self._describe_resources(
    ...
    @staticmethod
    def remove_temporary_test_config_file():
        try:
    ...
    @classmethod
    @log_time
    def clean_resources(cls, iris_project, resources_project, run_id, gce_zone):
    ...
    @classmethod
    @log_time
    def create_resources(cls,test_project, run_id, gce_zone):
...
if __name__ == "__main__":
    assert_root_path()
    IntegTest().deploy_test_uninstall()
