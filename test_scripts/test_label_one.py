...
def __instance_name():
    ...
    if not instance:
        raise ValueError("Must specify 'instance' key in environment for name of " \
...
def __parent_name():
    ...
    if not instance:
        raise ValueError("Must specify 'parent' key in environment for name of parent of the " \
...
def test_disks():
    ...
    label_one( project, __instance_name(), Disks().method_names()[0],
    )
...
def test_subscriptions():
    ...
    label_one(project, __instance_name(), Subscriptions().method_names()[0], __parent_name())
